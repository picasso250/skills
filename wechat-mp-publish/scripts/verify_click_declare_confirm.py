#!/usr/bin/env python3
"""
Reuse the declaration dialog and physically click the confirm button.
"""
...
CONFIRM_SELECTOR = "div.claim__original-dialog.original_dialog button.weui-desktop-btn.weui-desktop-btn_primary"
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.bring_to_front()
                locator = page.locator(CONFIRM_SELECTOR).first
                locator.wait_for(state="visible", timeout=10 * 1000)
                box = locator.bounding_box()
                if not box:
                    raise RuntimeError("Declaration confirm button was found but had no bounding box.")
                ...
                print(f"Confirm selector: {CONFIRM_SELECTOR}")
                print(
        ...
        logging.error("Failed while clicking declaration confirm in the reused editor tab: %s", exc)
        sys.exit(1)
