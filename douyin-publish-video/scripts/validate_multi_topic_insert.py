...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="validate_multi_topic_insert.py",
        description="Validate inserting multiple Douyin topics in sequence into description.",
    )
    parser.add_argument(
        "--word",
        action="append",
        dest="words",
        required=True,
        help="Topic word to type after ` #`. Pass multiple times for multiple topics.",
    )
    parser.add_argument(
        "--endpoint",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(Path.cwd() / "artifacts" / "douyin-topic-multi-insert"),
...
async def insert_one_topic(page: Page, word: str) -> dict:
    before_text = await get_editor_text(page)
    ...
    after_text = await get_editor_text(page)
    expected_fragment = f"#{word}"
    return {
        "word": word,
        "typed": typed,
        ...
        "inserted": expected_fragment in after_text,
    }


async def save_failure(page: Page, output_dir: Path) -> dict[str, str]:
    output_dir.mkdir(parents=True, exist_ok=True)
    shot_path = output_dir / "validate-multi-topic-insert-failure.png"
    meta_path = output_dir / "validate-multi-topic-insert-failure.json"
    ...
    async with async_playwright() as playwright:
        try:
            ...
            for word in args.words:
                print(f"STEP: insert topic {word}")
                step = await insert_one_topic(page, word)
                steps.append(step)

            final_text = await get_editor_text(page)
            result = {
                "url": page.url,
                "words": args.words,
                "steps": steps,
                "finalEditorText": final_text,
                "allInserted": all(step["inserted"] for step in steps),
            }
            print(f"RESULT_JSON:{json.dumps(result, ensure_ascii=False)}")
            return 0 if result["allInserted"] else 2
        except Exception:
            if page is not None:
