#!/usr/bin/env python3
"""
Reuse the WeChat article-editor tab and physically type into the author field.
"""
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.bring_to_front()
                locator = page.locator(AUTHOR_SELECTOR)
                locator.wait_for(state="visible", timeout=10 * 1000)
                box = locator.bounding_box()
                if not box:
                    raise RuntimeError("Author field was found but had no bounding box.")
                ...
                print(f"Author selector: {AUTHOR_SELECTOR}")
                print(
        ...
        logging.error("Failed while typing author in the reused editor tab: %s", exc)
        sys.exit(1)
