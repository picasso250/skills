...
and inspect the actual "开始创作" button only.
"""
...
def observe_start_button(page: Page) -> list[dict[str, object]]:
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.bring_to_front()
            results = observe_start_button(page)
            print(f"Editor tab title: {page.title()}")
            print(f"Editor tab url: {page.url}")
            print(f"Observed start buttons: {len(results)}")
