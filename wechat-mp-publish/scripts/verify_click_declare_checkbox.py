#!/usr/bin/env python3
"""
Reuse the declaration dialog and physically click the agreement checkbox icon.
"""
...
CHECKBOX_ICON_SELECTOR = "div.claim__original-dialog.original_dialog i.weui-desktop-icon-checkbox"
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.bring_to_front()
                locator = page.locator(CHECKBOX_ICON_SELECTOR).first
                locator.wait_for(state="visible", timeout=10 * 1000)
                box = locator.bounding_box()
                if not box:
                    raise RuntimeError("Declaration checkbox icon was found but had no bounding box.")
                ...
                print(f"Checkbox icon selector: {CHECKBOX_ICON_SELECTOR}")
                print(
        ...
        logging.error("Failed while clicking declaration checkbox in the reused editor tab: %s", exc)
        sys.exit(1)
