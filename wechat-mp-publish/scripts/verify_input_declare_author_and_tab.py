#!/usr/bin/env python3
"""
Reuse the declaration dialog, physically type the author, then press Tab.
"""
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.bring_to_front()
                locator = page.locator(DECLARE_AUTHOR_SELECTOR).first
                locator.wait_for(state="visible", timeout=10 * 1000)
                box = locator.bounding_box()
                if not box:
                    raise RuntimeError("Declaration author field was found but had no bounding box.")
                ...
                type_like_human(page, AUTHOR_TEXT)
                page.wait_for_timeout(150)
                page.keyboard.press("Tab")
                page.wait_for_timeout(600)
                after_value = locator.input_value()
                dialog_text = page.locator("div.claim__original-dialog.original_dialog div.weui-desktop-dialog").inner_text()
                print(f"Editor tab title: {page.title()}")
                print(f"Editor tab url: {page.url}")
                print(f"Declare author selector: {DECLARE_AUTHOR_SELECTOR}")
                print(
                ...
                print(f"Value after tab: {after_value!r}")
                print(f"Dialog text after tab: {dialog_text!r}")
            finally:
                browser.close()
    except Error as exc:
        logging.error("Failed while typing declaration author and tabbing: %s", exc)
        sys.exit(1)
