...
- **纯净性**：输入文本尽量使用中文，但允许少量英文缩写和单词。
- **原子性**：单次合成文本建议控制在 **100 个中文字**左右，**最多不超过 200 个中文字**。
...
- **黄金前三句 (The Golden Opening)**：视频的前三句话（或前 10 秒）**每句话必须至少包含一个 `*高亮关键词*`**。这是为了在开场即建立强烈的视觉刺激，提升观众的留存意愿。
- **数字语文化 (Verbalizing Numbers)**：
  - **语音金律**：**嘴里念的是中文大数，眼里看的是阿拉伯数字**。
  - **操作**：在生成语音阶段，脚本必须转换为自然的中文口语表述（如“二零二六年”、“两千两百多万”），且有效数字不得超过 2 位。
  - **字幕对齐**：在 `segments_final.json` 中，必须将 ASR 识别出的中文大数转换回**阿拉伯数字**（如“2026年”、“2200万”），以确保观众阅读的高效性。
  - **目的**：听得顺耳（降低认知负荷），看得明白（提升信息获取效率）。
